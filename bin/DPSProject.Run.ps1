...
function Show-MainForm_psf
{
	...
	$formMain_Load={
		...
		$formMain.ActiveControl = $buttonLoad
	}
	...
	$buttonSave_Click={
		...
		Add-JobTracker -Name 'Save' -ArgumentList $global:reportList, $global:datasetFile, $ScriptDirectory `
		-JobScript {
			#--------------------------------------------------
			...
			try
			{
				$path1 = $args[2] + "\bin\ReportIO.ps1"
				$path2 = $args[2] + "\data\MainDataset.dps"
				$path3 = $args[2] + "\backup\MainDataset.dps"
				. $path1
				write-reports -array $args[0] -path $args[1]
				Copy-Item $path5 $path6 -Force
			}
			catch
			{
	...
	$buttonUpdate_Click={
		...
		if ($currentDataset -eq "\MainDataset.dps" -and $global:changed -eq $true)
		{
			...
			if ($response.ToString() -eq "Yes")
			{
				write-reports -array $global:reportList -path $ScriptDirectory
			}
		}
		...
		-JobScript {
			...
			try
			{
				...
				$path4 = $args[0] + "\bin\DPSTextParser2.ps1"
				. $path1
				. $path2
				. $path3
				. $path4
...
AAAADwMAAAB2CgAAAk1TRnQBSQFMAgEBCAEAAXABAQFwAQEBEAEAARABAAT/ASEBAAj/AUIBTQE2
